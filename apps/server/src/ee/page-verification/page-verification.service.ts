...
export class PageVerificationService {
  ...
  async getVerificationInfo(dto: VerificationInfoDto, user: User) {
    ...
    if (!verification) {
      const perm = await this.getPermissions(dto.pageId, page.spaceId, null, null, user);
    ...
    return {
      ...
      verifiedBy: verification.verifiedById
        ? await this.getUserRef(verification.verifiedById)
        : null,
      expiresAt: verification.expiresAt?.toISOString() ?? null,
      requestedAt: verification.requestedAt?.toISOString() ?? null,
      requestedBy: verification.requestedById
        ? await this.getUserRef(verification.requestedById)
        : null,
      rejectedAt: verification.rejectedAt?.toISOString() ?? null,
      rejectedBy: verification.rejectedById
        ? await this.getUserRef(verification.rejectedById)
        : null,
  ...
  async createVerification(dto: CreateVerificationDto, user: User, workspace: Workspace) {
    ...
    if (dto.verifierIds.length > 0) {
      const verifierRows = dto.verifierIds.map((userId) => ({
        pageVerificationId: result.id,
        userId,
        addedById: user.id,
      }));
  ...
  async updateVerification(dto: UpdateVerificationDto, user: User, workspace: Workspace) {
    ...
    const mode = dto.mode ?? verification.mode;
    const periodAmount = dto.periodAmount ?? verification.periodAmount;
    const periodUnit = dto.periodUnit ?? verification.periodUnit;
    const fixedExpiresAt = dto.fixedExpiresAt;

    const expiresAt = this.calculateExpiresAt(
      mode as string | null,
      periodAmount,
      periodUnit as string | null,
      fixedExpiresAt,
    );
  ...
  async verifyPage(pageId: string, user: User, workspace: Workspace) {
    ...
    const canVerify = await this.userCanVerify(verification, user.id, workspace);
    ...
    if (verification.type === 'expiring') {
      newStatus = 'verified';
      newExpiresAt = this.calculateExpiresAt(
        verification.mode,
    ...
    const verifiers = await this.getVerifiers(verification.id);
    const otherVerifierIds = verifiers
      .map((v) => v.userId)
      .filter((id) => id !== user.id);

    if (otherVerifierIds.length > 0) {
      await this.notificationQueue.add(QueueJob.PAGE_VERIFIED_NOTIFICATION, {
        pageId: verification.pageId,
        spaceId: verification.spaceId,
        workspaceId: verification.workspaceId,
        actorId: user.id,
        verifierIds: otherVerifierIds,
      });
    }
  ...
  async submitForApproval(pageId: string, user: User) {
    ...
    const verifiers = await this.getVerifiers(verification.id);
    const verifierIds = verifiers.map((v) => v.userId);
  ...
  async rejectApproval(pageId: string, comment: string | undefined, user: User, workspace: Workspace) {
    ...
    const canReject = await this.userCanVerify(verification, user.id, workspace);
  ...
  async listVerifications(dto: VerificationsListDto, user: User, workspace: Workspace) {
    ...
    let query = this.db
      .selectFrom('pageVerifications as pv')
    ...
    if (dto.spaceIds && dto.spaceIds.length > 0) {
      query = query.where('pv.spaceId', 'in', dto.spaceIds);
    }

    if (dto.verifierId) {
      query = query
        .innerJoin('pageVerifiers as ver', 'ver.pageVerificationId', 'pv.id')
        .where('ver.userId', '=', dto.verifierId);
    }

    if (dto.type) {
      query = query.where('pv.type', '=', dto.type);
    }
    ...
    if (dto.beforeCursor) {
      const cursorRow = await this.db
        .selectFrom('pageVerifications')
        .select('id')
        .where('id', '=', dto.beforeCursor)
        .executeTakeFirst();

      if (cursorRow) {
        query = query.where('pv.id', '<', dto.beforeCursor);
        query = query.orderBy('pv.id', 'desc');
      }
    } else {
      if (dto.cursor) {
        query = query.where('pv.id', '<', dto.cursor);
      }
      query = query.orderBy('pv.id', 'desc');
    }
    ...
    const verifierRows = verificationIds.length > 0
      ? await this.db
          .selectFrom('pageVerifiers as pv2')
          .innerJoin('users as u', 'u.id', 'pv2.userId')
          .select(['pv2.pageVerificationId', 'u.id', 'u.name', 'u.avatarUrl'])
          .where('pv2.pageVerificationId', 'in', verificationIds)
          .execute()
      : [];

    const verifiersByVerificationId = new Map<string, any[]>();
    for (const row of verifierRows) {
      if (!verifiersByVerificationId.has(row.pageVerificationId)) {
        verifiersByVerificationId.set(row.pageVerificationId, []);
      }
      verifiersByVerificationId.get(row.pageVerificationId)!.push({
        id: row.id,
        name: row.name,
    ...
    const result = items.map((item) => ({
      ...
      status: this.computeEffectiveStatus(item as any) as any,
      mode: item.mode as string | null,
      periodAmount: item.periodAmount as number | null,
      periodUnit: item.periodUnit as string | null,
      verifiedAt: item.verifiedAt?.toISOString() ?? null,
      expiresAt: item.expiresAt?.toISOString() ?? null,
      createdAt: item.createdAt.toISOString(),
      pageTitle: getPageTitle(item.pageTitle),
      pageSlugId: item.pageSlugId,
  ...
  private async getPermissions(
    ...
    const isVerifier = verifiers?.some((v) => v.userId === user.id) ?? false;
    const canManage = isSpaceAdmin || isWorkspaceAdmin;

    let canVerify = false;
    let canSubmitForApproval = false;
    let canMarkObsolete = canManage;

    if (verification) {
      if (isVerifier || isWorkspaceAdmin) {
        if (verification.type === 'expiring') {
          canVerify = verification.status === 'draft' || verification.status === 'expired' || verification.status === 'verified';
        } else if (verification.type === 'qms') {
          canVerify = verification.status === 'in_approval' || verification.status === 'approved';
        }
      }

      if (verification.type === 'qms' && verification.status === 'draft') {
        canSubmitForApproval = true;
      }
    }

    return { canVerify, canManage, canSubmitForApproval, canMarkObsolete };
  }
  ...
  private async userCanVerify(verification: VerificationRow, userId: string, workspace: Workspace): Promise<boolean> {
    const verifierRow = await this.db
  ...
  private async getVerifiers(verificationId: string): Promise<VerifierRow[]> {
    return this.db
      .selectFrom('pageVerifiers as pv')
      .innerJoin('users as u', 'u.id', 'pv.userId')
      .select(['pv.id', 'pv.userId', 'u.name', 'u.avatarUrl', 'u.email'])
      .where('pv.pageVerificationId', '=', verificationId)
      .execute() as any;
  }
  ...
  private calculateExpiresAt(
    ...
    if (mode === 'period' && periodAmount && periodUnit) {
      ...
      switch (periodUnit) {
        ...
        case 'month': return new Date(now.setMonth(now.getMonth() + amount));
        case 'year': return new Date(now.setFullYear(now.getFullYear() + amount));
        default: return null;
      }
    }
    return null;
  ...
  private computeEffectiveStatus(verification: {
    status: string | null;
    type: string;
    expiresAt: Date | null;
  }): string {
    if (!verification.status) return 'none';
    if (verification.status === 'obsolete') return 'obsolete';
    if (verification.type === 'expiring' && verification.status === 'verified' && verification.expiresAt) {
      const now = Date.now();
      ...
      if (expiresMs - now <= windowMs) return 'expiring';
    }
  ...
  private async scheduleReconcile() {
    try {
      await this.notificationQueue.add(QueueJob.VERIFICATION_RECONCILE, {});
    } catch (err) {
      this.logger.error('Failed to schedule verification reconcile', err);
    }
  }
}
